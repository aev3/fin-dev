 ...
 * </p>
 */
public class BondForwardImpl implements LatticeIF
{
    /**
     * <p>
    ...
    public double[][] getLattice(Properties props)
    {
        ...
        return createLattice(rows, cols, q);
    }
     ...
     * @return
     */
    public double[][] createLattice(int rows, int cols, double q)
    {
         ...
         * Generate the last column with the value of the 'zcb_latt[n][n] - strike'
         * Last column:
         * Excel: =IF($A42 <=F$37,  F30-100*$B$34,"")
         */
        double coupon = 0.00;
        for(int row = rows - 1; row >= 0; --row)
        {
            int col = cols - 1;
            lattice[row][col] = zcb_latt[row][col] - 100.00 * coupon;
            --col;
        }
        /*
         * Generate data for all remaining columns where col <= cols-2:
         * Excel: r0,c4 =IF($A42 <=E$37, ($B$5*F41 + $B$6*F42)/(1+E16),""
         */
        for(int col = cols - 2; col >= 0; --col)
        {
            for(int row = 0; row <= rows-1; ++row)
            {
                if(row <= col)
                {
                    lattice[row][col] = (q * lattice[row + 1][col + 1]
                            + q_less * lattice[row][col + 1])
